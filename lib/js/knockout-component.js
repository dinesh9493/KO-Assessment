!(function ($) {
  !(function (factory) {
    ...
    var kc = typeof kcExports !== "undefined" ? kcExports : {};
    components = {};
    ...
    function bindComponentProperty(name, instance, binding, parentContext) {
      ...
      if (ko.isSubscribable(instance[name])) {
        if (ko.isWriteableObservable(binding))
          subscriptions.push(
            instance[name].subscribe(function (val) {
              if (blockMine) return;
              blockMine = true;
              binding(val);
              blockMine = false;
      ...
      if (ko.isSubscribable(binding)) {
        if (ko.isWriteableObservable(instance[name])) {
          subscriptions.push(
            binding.subscribe(function (val) {
      ...
      } else if (ko.isWriteableObservable(instance[name]))
        instance[name](binding);
    ...
    var templateSubscriptionDomDataKey =
      "__kc__templateSubscriptionDomDataKey__";
    ...
    ko.bindingHandlers.component = {
      init: function (
        ...
        if (typeof valueUnwrapped == "string") {
          ...
          throw new Error("component binding value must be component name");
        if (!components[name]) throw new Error("component not found " + name);

        var bindings = allBindingsAccessor(),
          componentBinding = bindings["componentBinding"],
          componentOptions =
            bindings["componentOptions"] || bindings["kc.options"],
          componentAssignTo =
            bindings["componentAssignTo"] || bindings["kc.assign"],
          subscriptions = [],
          oldInstance = ko.utils.domData.get(
            element,
            componentVMInstanceDomDataKey
          ),
        ...
        if (typeof componentAssignTo == "function")
          componentAssignTo(newInstance);
      ...
      update: function (
        ...
        var templateSubscription,
          componentVM = ko.utils.domData.get(
            element,
            componentVMInstanceDomDataKey
          ),
        ...
        if (components[name].templateReady)
          components[name].templateReady(function (template) {
            if (template) {
              templateSubscription = ko.renderTemplate(
                template,
                childBindingContext,
                bindings.templateOptions || {},
                element
              );
              disposeOldSubscriptionAndStoreNewOne(
                element,
    ...
    function Component(name, factory, template) {
      ...
      ko.bindingHandlers["kc." + name] = {
        init: function (
          ...
          return ko.bindingHandlers["component"].init(
            element,
            function () {
              return name;
            },
        ...
        update: function (
          ...
          var val = valueAccessor(),
            ...
            bindings = allBindingsAccessor();
          bindings.componentBinding = valueUnwrapped;
          return ko.bindingHandlers["component"].update(
            element,
            function () {
              return name;
            },
